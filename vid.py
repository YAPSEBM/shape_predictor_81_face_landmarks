...
def create_facestuff(image):
    ...
    fm, fm_rect, fm_ok = determine_facemarks(image, detector, predictor)
    if fm_ok and len(fm) == 81:
        fm = np.take(fm, [30,8,36,45,48,54], 0)
    else:
        return image
    marked_images_image = apply_facemarks(image, fm, (0,0,255))
    ...
    cv2.putText(image, str(ang2), tuple(x1), cv2.FONT_HERSHEY_SIMPLEX , 0.5, (255, 255, 128), 1)
...
while True:
    ret, frame = cap.read()
    cv2.imshow('Our Sketch', create_facestuff(frame))
